...
class Situation {
    ...
    void GetLookTarget(LookTarget& lt){
        for(uint i=0; i<known_chars.size(); ++i){
            MovementObject@ mo = ReadCharacterID(known_chars[i].id);
            
            vec3 char_pos = mo.position;
            float dist = distance(this_mo.position, char_pos);
            if(dist <= 0)
                dist = 1.0f;
            known_chars[i].interest = 1.0f / dist;
